...
for i in range(start, end, step):
    ...
    u_ex = df.Expression("1 + x[0] + 2*x[1]", degree=6, domain=mesh)

    Z_N = df.VectorFunctionSpace(mesh, "CG", degV, dim=2)
    ...
    auv = (
        ...
        * dx(Neumann)
        ...
        - df.inner((1.0 + u * u) * df.grad(u), n) * v * (ds(Dirichlet))
    )
...
for i in range(start, end, step):
    ...
    F = (
        df.inner((1 + u**2) * df.grad(u), df.grad(v)) * df.dx
        - f * v * df.dx
        - g * v * ds(Neumann)
    )
