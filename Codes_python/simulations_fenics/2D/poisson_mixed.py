...
for i in range(start, end, step):
    ...
    for cell in df.cells(mesh):  # all triangles
        for facet in df.facets(cell):  # all faces
            ...
            if phi(v1.point()) * phi(v2.point()) <= 0.0 or df.near(
                ...
                elif neumann(v1.point()) and neumann(v2.point()):
                    Cell[cell] = Neumann
    ...
    Z_N = df.VectorFunctionSpace(mesh, "CG", degV, dim=mesh.topology().dim())
    ...
    gamma_div, gamma_u, gamma_p, sigma_N, gamma_D, sigma_D, sigma_p = (
        ...
        0.20,
        0.0001,
    )  # 10.0, 1.0, 100.0, 1.0, 10000.0, 100.0, 0.01
    ...
    boundary_penalty = (
        ...
        * (dS(Dirichlet) + dS(Interface))
        + sigma_D
        * h**2
        * (df.inner(df.div(df.grad(u)), df.div(df.grad(v))))
        * (dx(Dirichlet) + dx(Interface))
    )
    ...
    auv = (
        ...
        - df.inner(df.dot(df.grad(u), n), v) * (ds(Dirichlet) + ds(Interface))
    )
    ...
    lq_D = -gamma_D * h ** (-2) * df.inner(u_D, q_D * phi) * dx
