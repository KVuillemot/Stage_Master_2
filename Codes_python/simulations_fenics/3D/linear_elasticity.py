...
def Sigma(u):
    return lmbda * df.div(u) * df.Identity(3) + 2.0 * mu * epsilon(u)
...
for i in range(start, end, step):
    ...
    background_mesh = df.BoxMesh(df.Point(-0, 0, 0), df.Point(1, 1, 1), H, H, H)
    ...
    u_ex = df.Expression(
        ("cos(x[0])*sin(x[1])", "sin(x[0])*cos(x[1])", "sin(x[0])*cos(x[2])"),
        degree=3,
        domain=mesh,
    )
